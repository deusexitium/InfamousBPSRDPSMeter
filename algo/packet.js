...
const AttrType = {
    ...
    AttrHp: 0x2c2e,
    AttrMaxHp: 0x2c38,
    AttrHaste: 0x2b84, // Haste attribute
    AttrMastery: 0x2b8e, // Mastery attribute
...
class PacketProcessor {
    ...
    _processAoiSyncDelta(aoiSyncDelta) {
        ...
        for (const syncDamageInfo of skillEffect.Damages) {
            ...
            if (isTargetPlayer) {
                ...
                // CRITICAL: Reset HP when player is healed (respawn/revival detection)
                if (isHeal && damage.toNumber() > 0) {
                    const target = this.userDataManager.getUser(targetUuid.toNumber());
                    if (target && target.attr && target.attr.hp === 0) {
                        // Player was dead (HP=0) and just got healed, likely a respawn
                        // Set HP to a small value to indicate alive again
                        this.userDataManager.setAttrKV(targetUuid.toNumber(), 'hp', damage.toNumber());
                    }
                }
            } else {
                //非玩家目标
    ...
    async _processSyncToMeDeltaInfo(payloadBuffer) {
        ...
        if (uuid && !currentUserUuid.eq(uuid)) {
            const oldLocalPlayerUid = currentUserUuid.shiftRight(16).toNumber();
            currentUserUuid = uuid;
            const newLocalPlayerUid = currentUserUuid.shiftRight(16).toNumber();
            
            this.logger.info('Got player UUID! UUID: ' + currentUserUuid + ' UID: ' + newLocalPlayerUid);
            ...
            // Update local player UID in data manager for solo mode
            if (this.userDataManager && newLocalPlayerUid > 0) {
                this.userDataManager.setLocalPlayerUid(newLocalPlayerUid);
            }
        }
        ...
        for (const attr of attrs) {
            ...
            switch (attr.Id) {
                case AttrType.AttrName:
                    ...
                    this.logger.info(`✅ CAPTURED NAME: ${playerName} for UID ${playerUid}`);
                    console.log(`✅ CAPTURED NAME FROM PACKET: ${playerName} (UID: ${playerUid})`);
                ...
                case AttrType.AttrHaste:
                    const playerHaste = reader.int32();
                    this.userDataManager.setAttrKV(playerUid, 'haste', playerHaste);
                    break;
                case AttrType.AttrMastery:
                    const playerMastery = reader.int32();
                    this.userDataManager.setAttrKV(playerUid, 'mastery', playerMastery);
                    break;
